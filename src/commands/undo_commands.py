...
class AddTextCommand(QUndoCommand):
    """テキスト追加のUndoコマンド"""
    def __init__(self, scene, text_elem, item):
        super().__init__("テキストを追加")
        self._scene = scene
        self._text_elem = text_elem
        self._item = item

    def redo(self):
        if self._text_elem not in self._scene._page.text_elements:
            self._scene._page.text_elements.append(self._text_elem)
        if self._item.scene() != self._scene:
            self._scene.addItem(self._item)
        self._scene.page_modified.emit()

    def undo(self):
        if self._text_elem in self._scene._page.text_elements:
            self._scene._page.text_elements.remove(self._text_elem)
        if self._item.scene() == self._scene:
            self._scene.removeItem(self._item)
        self._scene.page_modified.emit()
